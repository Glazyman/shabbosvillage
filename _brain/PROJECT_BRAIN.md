...
- **Owner manual steps (pending):** (1) add the 3 env vars in Vercel; (2) create the Stripe webhook endpoint `https://shabbosvillage.com/api/stripe-webhook` for `checkout.session.completed` + `checkout.session.expired`, copy signing secret. Group-rental holds are confirmed manually in the Supabase dashboard for now (no admin UI yet).
