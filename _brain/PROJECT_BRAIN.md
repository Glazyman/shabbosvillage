...
### 2026-06-22 — Gallery: first real Shabbos Village photos

- Replaced 4 of the 9 Unsplash placeholders in `app/gallery/page.tsx` with real campground photos (owner-supplied, from `~/Downloads` ChatGPT-named PNGs). Converted each from ~3 MB 1086×1448 PNG → optimized JPEG (`sips`, quality 82, 1000px wide, ~500–650 KB) and committed to **`public/gallery/`**: `entrance.jpg` (welcome sign + driveway), `bathhouse.jpg` (shower/toilet cabins), `rv-site.jpg` (RV at a wooded site), `gathering.jpg` (volleyball court + fire pit). Served via `next/image` (further optimized on delivery).
- Slot mapping: the two most-portrait subjects (entrance, RV) took the `tall` grid slots; bathhouse + gathering took `normal` slots. New captions: "Welcome to the Village", "Showers & Toilets", "RV Sites", "Volleyball & Fire Pit". Kept 5 Unsplash placeholders (Forest Sites, Evening Fires, Starry Nights, Nature & Trails, Rest & Relax).
- Updated the intro + footer notes (previously claimed *all* photos were placeholders) to reflect the now-mixed real/preview set. Hero image left as the existing Unsplash shot (in scope was the grid) — the `entrance.jpg` would make a good hero swap later if wanted.
- Verified `npx next build` (all 13 routes). Original PNGs in `~/Downloads` were left untouched.
